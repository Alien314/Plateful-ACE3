...
[{
    ...
    if (!isGamePaused) then {
        ...
        if ((CBA_missionTime - _lastIntensityUpdate) >= INTENSITY_UPDATE) then {
            ...
            if (local _unit) then {
                if (_isThisUnitAlive) then {
                    if !(_unit getVariable ["ACE_isUnconscious", false] || {lifeState _unit == "INCAPACITATED" || {_unit getVariable ["unconscious", false]}}) then {
                        if !(isPlayer _unit) then {
                            ...
                            if ((_unit isEqualTo vehicle _unit) && (_sdr || ({ 0.05 > random 1 }))) then {
								...
								if (isNil QGVAR(burnRun)) then {
								  _group setVariable [QGVAR(burnRun), speedMode _group]; };
                    ...
                    if (GVAR(aceMedLoaded)) then {
					  ...
					  if !(isDamageAllowed _unit) exitWith {};
						private _woundSelection = ["Face", "Neck", "Head", "Pelvis", "Abdomen", "Diaphragm", "Chest", "Arms", "Hands", "Legs"] selectRandomWeighted [0.22, 0.15, 0.4, 0.06, 0.2,0.04, 0.2, 0.8, 0.8, 0.6];
						private _burnDamage = ((_intensity / MAX_INTENSITY) * GVAR(fireMult));
    ...
    if (local _unit) then {
        if (!isPlayer _unit) then {
			...
			if ( ( (units _group) findIf {_x getVariable [QGVAR(burning), false]}) == -1) then {
			    _group setSpeedMode (_group getVariable[QGVAR(burnRun), "UNCHANGED"]);
				_group setVariable[QGVAR(burnRun), nil];
			};
